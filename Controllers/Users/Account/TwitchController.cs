    ...
    public class TwitchController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> Validating_Twitch_Login([FromBody] Validate_TwitchDTO dto)
        {
            try
            {
                ...
                string twitch_access_token = Twitch.Get_Access_Token(dto.Code).Result;
        ...
        [HttpPost("Integrate")]
        public async Task<ActionResult<string>> Integrate_Twitch_Email([FromBody] Validate_TwitchDTO dto)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                bool user_twitch_id_exists = Users_Repository.ID_Exists_In_Twitch_IDsTbl(user_twitch_id).Result;
                bool twitch_email_exists = Users_Repository.Email_Exists_In_Twitch_Email_AddressTbl(user_email).Result;
                bool user_email_exists_in_login_email_addresstbl = Users_Repository.Email_Exists_In_Login_Email_AddressTbl(user_email).Result;

                if (user_twitch_id_exists || twitch_email_exists || user_email_exists_in_login_email_addresstbl) {
                    return BadRequest();
                }

                await Users_Repository.Integrate_Account_By_Twitch(new Complete_Twitch_RegisterationDTO
                {
